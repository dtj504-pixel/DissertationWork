...
Some ideas for further development could be moving to a more complicated dataset; applying the framework to an example with more stocks to determine whether it still works well in high dimensions; doing the risk calculation when there are iterations (i.e. the standard way MixME suggests); modelling the uncertainty of the SSB values outputted by the simulation as talked about earlier on and considering trophic interactions [@MixMEwiki].
