...
The discrepancy in the $F_{cod}$ given by the shortcut model and those suggested by @ICES2025CodAdvice could be due to ICES maximising the long-term single-stock catch for cod whilst my simulation aims to maximise the long-term total catch for both cod and haddock. This could lead to the quota for cod being raised, whilst remaining precautionary, so that we can fish more haddock. Allowing this could result in a lower long-term catch for cod, but this reduction in cod catch could be made up for by the increase in haddock catch. Thus, the model may be trying to balance the trade off of reducing the long term catch of cod by fishing above the $F_{MSY}$ for cod with catching more haddock if it does this. This would result in a value between $F_{MSY}$ and $F_{PA}$ for cod which is exactly what we see above.

The simulation could also be unrealistic due to using different types of gear in different regions. This means, for example, that the ratio of cod to haddock caught by the fleets used in the simulation could be very different than those that actually operate in the North Sea. Thus, the fleets would have to stop fishing earlier than expected.
...
Some ideas for further development could be moving to a more complicated dataset; apply the framework to an example with more stocks to determine whether it still works well in high dimensions; doing the risk calculation when there are iterations (i.e. the standard way MixME suggests); model the uncertainty of the SSB values outputted by the simulation as talked about earlier on and considering trophic interactions [@MixMEwiki].
