---
title: "Accelerating Management Strategy Evaluation Grid-Searches Using Bayesian History Matching, Acquisiton Functions and K-Means Clustering"
author: "Emma Bowen"
format: 
  html:
    toc: true
...
This project introduces a computationally efficient framework for evaluating HCRs [@Originlapaper]. Initially applied to a single-stock fishery, the framework utilises Gaussian process regression to independently model both catch and risk [@Originalpaper]. Then, by combining the methods of Bayesian history matching, acquisition functions and k-means clustering, we are able to systematically map the sample space [@Originalpaper],[@BayesianOptimisationTutorial],[@AKMeansClusteringAlgorithm]. This drastically reduces the need for expensive simulation runs [@EfficientGlobalOptimizationofExpensiveBlackBoxFunctions].

Building on this foundation, we scale up this framework to complex mixed-fisheries scenarios [@MixME]. Here, the objective becomes identifying the fishing mortality for each of the interacting stocks that safely maximizes total long-term catch [@MixedFisheriesStatusandManagement2023],[@ULRICH201238]. This framework provides a powerful tool for marine policy by significantly reducing the computational burden of mixed-fishery MSEs whilst ensuring that the stocks remain both economically viable and firmly within safe biological limits [@Originalpaper],[@ICES2019WKGMSE2],[@Harvest_Control_Rules].

# Introduction

MAYBE A BIT MORE DEFINING OF TERMS LIKE IN MIKE'S? OR A BIT MORE MOTIVATION - ASK GUSTAV

This paper describes the framework created above in detail with applications to a couple of scenarios, the results of which are discussed in @sec-analysingresults. We also suggest areas for further development of the method in @sec-conclusionandfurtherdevelopment.

In the @sec-singlestock part of the project, we are building on the @Originalpaper paper. This paper is looking to find the HCR parametrised by $F_{target}$ and $B_{trigger}$ that maximises the median long-term catch whilst keeping the risk precautionary for a single-stock fishery [@Originalpaper]. The paper uses many different methods to achieve this, which we investigate in detail below and apply to our own work. We have focused on improving these methods by adding acquisition functions and k-means clustering to improve how we choose to sample points between rounds as this was not deeply considered in the original paper [@Originalpaper],[@BayesianOptimisationTutorial],[@EfficientGlobalOptimizationofExpensiveBlackBoxFunctions],[@GlobalOptimizationofStochasticBlackBoxSystemsviaSequentialKrigingMetaModels]. We demonstrate clearly how these methods become applicable in our own work in @sec-application.

The aim of the second half of the project in @sec-mixedfisheries is to apply the methods from @sec-singlestock to mixed fisheries. We use the methods from the first half of the project but change the aim to finding the $F_{target}$ for each stock that is precautionary but maximises long-term catch. We focus on two datasets and receive results for each by running the simulations in parallel on a High Performance Computer because the objective function is no longer pre-evaluated as it was in @Originalpaper. This means we have used a much more realistic example and have been able to demonstrate how effective our method may be if used in the industry [@BayesianOptimisationTutorial],[@MixME],[@MixedFisheriesStatusandManagement2023].

Furthermore, we set up a new objective function to maximise in @eq-objectivefunctionmultstocks and a new risk calculation. We claim that this method of risk calculation satisfies the ICES precautionary standard [@ICES2019WKGMSE2]. Our new risk calculation was mostly developed due to our second dataset having only on iteration, and so is not necessarily generally applicable. There is more discussion about this in @sec-conclusionandfurtherdevelopment.

# Creating the framework in a single stock fisheries context {#sec-singlestock}

## Context

We have the objective function @eq-objectivefunctionsinglestock in the paper which combines the risk and the catch and so this is the function we want to maximise to get our maximum median long-term catch with the constraint of keeping the risk below 0.05. To do this, the paper takes a Bayesian History Matching (BHM) approach. Firstly, we sample our first eight points which are spaced evenly throughout the sample space to get some initial data. Then, for each round we do the following:
...
We are sampling eight points per round to reduce computation time [@HumanOutOfTheLoopBayesian],[@Originalpaper]. Now, we will look at the mathematics behind the methods above and also at the acquisition functions which I added to the original code.
...
Recall from @sec-setupGPs that we have the GP for $\ln(median\ catch)$ which is modeling the $C(\theta)$ part of the objective function. As the first half of the objective function $f$ is an indicator function, we can focus on maximising $C(\theta)$ whilst meeting the threshold stated by the indicator function [@BayesianOptimisationTutorial],[@EfficientGlobalOptimizationofExpensiveBlackBoxFunctions].
...
For Expected Improvement, we use the @eq-EIincode expression and calculate the EI for every point in the sample space. For Augmented Expected Improvement, as our situation has no noise, in our code we are still using $f^*_n$ [@Originalpaper],[@BayesianOptimisationTutorial],[@GlobalOptimizationofStochasticBlackBoxSystemsviaSequentialKrigingMetaModels]. Thus, we use the equation:

$$
AEI_n(x_{n+1})=EI_n(x_{n+1})\Biggl(1- \frac{\sigma_{obs}}{\sqrt{\sigma_n^2(x_{n+1})+\sigma_{obs}^2}}\Biggr)
$$

to calculate the AEI for every point in the sample space, where we set $\sigma_{obs} = 0$. For Knowledge Gradient, we estimate the KG from 100 simulations and update the KG each round using @eq-updatemuKG .
...
Due to the change in stocks being modelled, I have decided to keep the GP prior for catch modelled as the same form as in @eq-gpmeanrisk. Despite the more complicated prior used in later rounds for the catch GP in @Originalpaper being designed to approximate yield curves, it may not be appropriate in a mixed fisheries context where the catch of one species is affected by the catch of another [@Originalpaper],[@MixME],[@ULRICH201238]. Leaving the GP more general avoids mis-specification, ensuring that the GP can be appropriately fitted to the points [@williams2006gaussian].

The process for the first half of the project adapted to our new situation is outlined below. Firslty, note that we are now seeking to maximise the objective function below:

$$
\small
g(\theta) = I_{[Cod\ B_{lim},\infty]}P(SSB_{Cod}<B_{lim}) \times I_{[Haddock\ B_{lim},\infty]}P(SSB_{Haddock}<B_{lim}) \times C(\theta)
$$ {#eq-objectivefunctionmultstocks}

where the indicator functions are defined as beforehand, except using these new sets. $Cod\ B_{lim}$ and $Haddock\ B_{lim}$ are the $B_{lim}$ for cod and haddock respectively and $SSB_{Cod}$ and $SSB_{Haddock}$ are the $SSB$ for cod and haddock respectively. This follows on intuitively from the @eq-objectivefunctionsinglestock once we consider that we are now modelling for multiple stocks and want to maximise the catch whilst keeping both stocks safe.

Firstly, to get some initial data, we randomly sample our first set of $n-1$ points, where $n$ is the number of cores the system we are on has. This is again done to reduce computation time [@Originalpaper],[@HumanOutOfTheLoopBayesian]. Then, for each round we do the following:
...
From running our simulation for the shortcut model on the Viking HPC, we were able to obtain some results [@VikingDocumentation]. The shortcut model initially gave us a long list of parameters, each with $F_{cod} = 0.42$ and $F_{had} \ge 0.34$. As $F_{had} = 0.34$ was given as an optimal parameter in the list, it means that this results in the same long-term catch of haddock as if we were to fish at any $F_{had} > 0.34$ as we were maximising total long-term catch. Remembering that cod is the choke stock, this list also suggests that when we fish at $F_{cod} = 0.42$ this causes us to fish at $F_{had} = 0.34$ due to the ratio of cod to haddock that we are catching across the fleets [@MixMEwiki]. Thus, $F_{cod} = 0.42$ and $F_{had} = 0.34$ are the optimal parameters suggested by the shortcut simulation.

These appear to be relatively high. ICES documents regularly suggest values lower values for $F_{cod}$ in the North Sea region for their $F_{MSY}$ values [@ICES2019CodAdvice],[@ICES2020CodBlim]. Looking at the most recent advice, the model's output of $F_{cod}=0.42$ is significantly above the $F_{MSY} = 0.231$ but below $F_{PA} = 0.464$ [@ICES2025CodAdvice]. - FIND THE DEFINITIONS OF THESE AND EXPLAIN WHETHER THIS IS BAD OR NOT

On the other hand, for haddock the optimal parameter given the shortcut model $F_{had} = 0.34$ is below the $F_{MSY} = 0.353$ and the $F_{PA} = 0.708$ [@ICES2025HaddockAdvice]. This shows that the shortcut model has produced a safe value for $F_{had}$ according to the latest ICES guidelines.

highlight how stocks are in different regions and so toy model doesn't really match real data.

discuss results form first half briefly - say my method still works basically - before mixme results

# Conclusion and Further areas for Development {#sec-conclusionandfurtherdevelopment}

MAYBE A BIT TOO GENERAL AT THE END OR NOT ENOUGH ELABORATION ON CONCEPTS IN THE PAPER?
