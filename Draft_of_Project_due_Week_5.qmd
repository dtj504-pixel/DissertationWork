...
MAYBE A BIT MORE DEFINING OF TERMS LIKE IN MIKE'S?

FIND CITATIONS THROUGHOUT
...
highlight how stock s in different regions and toy model so doesn't really match real data. also ices doesn't use mixme.
...
We have created a framework that is able to speed up management strategy evaluation grid searches in single stock or multi-stock contexts, significantly reducing the burden of expensive runs of the objective function. This was done by using advanced statistical techniques such as BHM and Gaussian Process Regression as well as providing some of our own insights by using acquisition functions and k-means clustering. We have compared the usefulness of different acquisition functions and decided upon one to implement in our framework. We have also thoroughly explained how the addition of k-means clustering improves the trade off between exploration and exploitation when sampling multiple points. In addition, we have successfully demonstrated the usefulness of this model by showing it takes less evaluations than in @Originalpaper to converge on the optimal point for the Case Study and by applying our method to a dataset with multiple stocks.

Overall, this framework presents a more computationally efficient way for stakeholders to be able determine management strategies that will keep stocks healthy whilst maximising the long-term catch of the fishery. This is especially relevant at this time because many mixed fisheries are struggling after the collapse of an individual stock causes them to have to severely limit catch of others despite growing demand from consumers. By reducing computation time for MSE grid searches, we make it easier for more simulations to be run, which could allow fisheries advice to be published in a more timely manner. This could allow organisations such as ICES to be able to keep pace with the changes in environment, interactions between species and early maturation. As a result, fisheries advice will be more accurate and relevant, contributing towards a healthier ecosystem and planet.
