...
The discrepancy in the $F_{cod}$ given by the shortcut model and those suggested by @ICES2025CodAdvice could be due to ICES maximising the long-term single-stock catch for cod whilst my simulation aims to maximise the long-term total catch for both cod and haddock. This could lead to the quota for cod being raised, whilst remaining precautionary, so that we can fish significantly more haddock. If haddock biomass is large, then allowing the fleet to fish $X$ tonnes more cod could result in the bycatch of $Y$ tonnes more haddock with $Y > X$. Thus, the model may be trying to balance the trade off of reducing the long term catch of cod by fishing above the $F_{MSY}$ for cod with catching significantly more haddock if it does this. This would result in a value between $F_{MSY}$ and $F_{PA}$ for cod which is exactly what we see above.
