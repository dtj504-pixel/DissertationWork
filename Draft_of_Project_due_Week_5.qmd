...
For Expected Improvement, we use the @eq-EIincode expression and calculate the EI for every point in the sample space. For Augmented Expected Improvement, as our situation has no noise, in our code we are still using $f^*_n$ [@Originalpaper],[@BayesianOptimisationTutorial]. Thus, we use the equation:
