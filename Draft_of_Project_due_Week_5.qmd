...
The mean was also identified as a possible issue. By assuming a very specific mean (shown below @eq-gpmean) for the Gaussian Process (GP) we are assuming that we know a lot about the objective function $f$ (which is what we are modeling with our GP) [@williams2006gaussian]. Thus, it was suggested that I change the code to a zero mean which is commonly done when we do not want to make any assumptions about our objective function [@williams2006gaussian].
...
![Figure 1: Exponential kernel](images/case_study8_exp_kernel_7_rounds.png){alt="Figure 1: Exponential kernel"}

![Figure 2: Gaussian kernel](images/case_study8_gauss_kernel_7_rounds.png){alt="Figure 2: Gaussian kernel"}

![Figure 3: Matern 5/2 kernel](images/case_study_matern5_2_kernel_7_rounds.png){alt="Figure 3: Matern 5/2 kernel"}
...
![Figure 4: case_study8.R with EI at seven rounds](images/ei_looped_exp_kernel_round_7.png){alt="Figure 4: case_study8.R with EI at seven rounds"}

![Figure 5: case_study8.R with EI using Gauss kernel at seven rounds](images/looped_ei_gauss_kernel_7_rounds.png){alt="Figure 5: case_study8.R with EI using Gauss kernel at seven rounds"}

![Figure 6: Last round of case_study8.R with EI function using Matern 5/2 kernel](images/looped_EI_matern_5_2_non_converge.png){alt="Figure 6: Last round of case_study8.R with EI function using Matern 5/2 kernel"}

### Experimenting with the mean

Setting the mean of the GP to zero as in @lst-mean_zero is more often done in examples to showcase the key concepts than when implementing these methods in real-world situations [@williams2006gaussian]. My experiments seem to back up this theory as they again show results that are the same or worse than our original approach.

``` {#lst-mean_zero .R lst-cap="Setting the GP Mean to Zero"}
...
![Figure 7: case_study8.R with the mean of the GP set to zero in the first round only at the seventh round](images/mean_zero_first_roudn_only.png){alt="Figure 7: case_study8.R with the mean of the GP set to zero in the first round only at the seventh round"}

![Figure 8: case_study8.R with the mean of the GP set to zero in all rounds at the seventh round](images/mean_zero_all_rounds.png){alt="Figure 8: case_study8.R with the mean of the GP set to zero in all rounds at the seventh round"}

![Figure 9: case_study8.R augmented with the KG acquisition function with the mean of the GP set to zero in the first round at the seventh round](images/looped_kg_mean_zero_first_round_after_7_rounds.png){alt="Figure 9: case_study8.R augmented with the KG acquisition function with the mean of the GP set to zero in the first round at the seventh round"}

![Figure 10: case_study8.R augmented with the KG acquisition function with the mean of the GP set to zero in all rounds at the seventh round](images/looped_kg_mean_zero_every_round_7_rounds.png){alt="Figure 10: case_study8.R augmented with the KG acquisition function with the mean of the GP set to zero in all rounds at the seventh round"}
