...
Maybe the discrepancy is the environmental conditions or fishing pattern put into the mixme model and those form the data ICES has - "a given fishing pattern and current environmental conditions" in the definition of $F_{MSY}$.

BELOW TWO PARGRAPHS MAKES SENSE IF HADDOCK BIOMASS SIGNIFICANTLY LARGER THAN COD BIOMASS - as then even with the ratio of 0.34 to 0.42 getting the extra, say 0.05 of haddock may actually become very significant of there are 1 million tonnes of haddock but only 30 tonnes of cod.

This large discrepancy in the $F_{cod}$ given by the shortcut model and those suggested by @ICES2025CodAdvice could be due to ICES maximising the long-term single-stock catch whilst my simulation aims to maximise the long-term total catch for both cod and haddock. This could lead to more cod being fished, whilst remaining precautionary, so that we can fish significantly more haddock?? - WOULDN'T THIS SAME LOGIC LEAD TO FISHING MORE COD JUST TO FISH MORE COD?

SIMILAR TO ABOVE - Another reason could be fishing at a higher $F_{cod}$ than expected, but still within the safe $F_{PA}$ limit, so that we can catch more haddock as well as more cod. - THOUGHT MIXME STUCK TO STOPPING AT THE CHOKE SPECIES - IT'S A MIN SETTING SOMEWHERE - I'm setting the quota for cod to be 0.42, the min is not automaticalaly set at F_MSY
