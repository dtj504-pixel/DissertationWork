...
-   We select 8 plausible points to sample in the next round - BRIEFLY JUSTIFY, MOSTLY ON COMPUTATION TIME, WHY I SAMPEL EIGHT POINTS EACH ROUND
...
Firstly, to get some initial data, we randomly sample our first set of $n-1$ points, where $n$ is the number of cores the system we are on has [@Originalpaper]. Then, for each round we do the following:
