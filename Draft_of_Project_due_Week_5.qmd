...
$Ftrgt$ refers to $Ftarget$ in all figures in this section. - BEST TO CHANGE IF POSS
...
$$
g(\theta) = I_{[Cod\ B_{lim},\infty]}(P(Cod\ SSB<B_{lim})) \times I_{[Haddock\ B_{lim},\infty]}(P(Haddock\ SSB<B_{lim})) \times C(\theta)
$$ {#eq-objectivefunctionmultstocks}

where the indicator functions are defined as beforehand, except using these new sets. Thihs follows on intuitively from the @eq-objectivefunctionsinglestock once we consider that we are now modelling for multiple stocks and want to maximise the catch whilst keeping both stocks safe.
