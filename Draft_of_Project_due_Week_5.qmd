...
Throughout this section, we have assumed that the $SSB$ for each stock for each year outputted by the simulation is the true value. In reality there may be some uncertainty about this value and so we would need to take this into account [@MixME]. This would still allow $\textrm{min}(SSB)$ to be modelled as a GP and the risk calculation to be the same so that the rest of the process can be carried out as above. However, this is beyond the scope of my project and so I have not implemented this and will not discuss it in detail.
