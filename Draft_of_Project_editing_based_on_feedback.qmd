...
In the literature, this method can be understood through the lens of Constrained Bayesian Optimisation [@letham2018constrainedbayesianoptimizationnoisy],[@HumanOutOfTheLoopBayesian]. We are using BHM to determine which points are possible based on:
...
We have three different acquisition functions we have investigated using here. We set the value for each acquisition function to zero if the point doesn't meet the precautionary threshold [@letham2018constrainedbayesianoptimizationnoisy].
...
We have now been able to determine which points are possible and have then assigned a value to each point in $PS$ using one of the acquisition functions above. Now, we want to decide which 8 points are best to evaluate next.
...
-   We set up or update the GPs to model the $\textrm{min}(SSB)$ from 2030-2039 for each stock and the GP to model the total catch from 2030-2039
