...
and where @eq-MultivariateNormalDist can be referred to as the prior distribution of the GP, or simply the prior.
...
When using GPs to emulate our objective function, we need to be able to estimate the hyperparameters of the GP using the data we gain from evaluating our points $x_1,...,x_n$. We can do this as follows. Firstly, we let the vector $\eta$ represent the hyperparameters of the GP. Then, given the observations $f(x_{1:n}) = (f(x_1),...,f(x_n))$, we calculate the likelihood of these observations under the prior distribution of the GP given $\eta$ which is denoted as $p(f(x_{1:n})|\eta)$ and modelled by @eq-MultivariateNormalDist. Lastly, we set $\hat{\eta}$ to the value that maximizes this likelihood:
...
-   We set up or update the Gaussian Process (GP) to model $\ln(risk)$ and the GP to model $\ln(median\ long\textrm{-}term\  catch)$
...
-   We use the GP which is modelling $\ln(median\ long\textrm{-}term\ catch)$ to get the value for the median catch at every point in the parameter space, which will have some uncertainty
...
The mean functions of the GPs $\textrm{min}(SSB_{Cod})$ and $\textrm{min}(SSB_{Haddock})$ have remained modelled as in @eq-gpmeanrisk from the first half of the project to avoid mis-specification [@williams2006gaussian],[@DiceKrigingPaper]. The GPs have kept the same estimation method because Maximum Likelihood Estimation is one of the standard methods used in the literature for our situation [@williams2006gaussian],[@BayesianOptimisationTutorial]. We also keep the small nugget term as our simulation is still deterministic and we still need our matrices to be invertible [@DiceKrigingDocumentation],[@williams2006gaussian],[@BayesianOptimisationTutorial].

The $\textrm{min}(SSB)$ for each stock is not necessarily smooth. Due to our discrete time steps, overfishing or low recruitment can cause sudden drops [@MixMESupp],[@ICES2021precautionary]. These cannot easily be modelled by any of the kernels we have previously explored due to the size of these drops [@williams2006gaussian]. However, to maintain consistency between the first and second halves of this project and as $min(SSB)$ is on a similar scale to $C(\theta)$ , we choose to continue using the exponential kernel for the GPs in this half of the project.
...
This can be done using @eq-posteriordistributiongivensamples. A diagram is provided below to improve visualisation, where cross-section means we are taking the GP for a specific value of $F_{had}$:

![](images/Calculating%20risk%20demonstration_21_04_1.drawio.png)
