...
The $\textrm{min}(SSB)$ for each stock is not necessarily smooth. Due to our discrete time steps, overfishing or low recruitment can cause sudden drops [@MixMESupp],[@ICES2021precautionary]. These cannot easily be modelled by any of the kernels we have previously explored due to the size of these drops [@williams2006gaussian]. So, to maintain consistency between the first and second halves of this project, we choose to continue using the exponential kernel for the GPs in this half of the project.
