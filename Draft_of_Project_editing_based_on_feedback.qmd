...
Furthermore, as there are 1000 iterations in this dataset, I will then be able to do the risk calculation as described in @MixME and as is it is typically done across the industry [@MixMESupp],[@ICES2019WKGMSE2]. As we have multiple iterations, we can follow the MixME definition that risk is the proportion of iterations in each year where $SSB < B_{lim}$, i.e. $P(SSB<B_{lim})$ where the probability is taken over the number of iterations [@MixMEwiki].
...
A last thing to consider would be modeling the uncertainty of the $SSB$ values outputted by the MixME simulation and including this uncertainty in our GPs. MixME gives us this pre-computed uncertainty - WHERE AND DOES IT?? GUSTAV MADE A COMMENT ABOUT THIS SOMEWHERE

We would include the uncertainty for the $SSB$ value at each point in the GP as an appropriate input. - IS IT ACTUALLY AN OVERALL OBSERVATIONO ERROR? THINK THIS IS INDICATED BY DICEKRIGING PACKAGE

Then, the GP would take this into account when modelling $SSB$ and so we would be able to estimate the $SSB$ value for each point in the grid from the GP as normal. It is important to note that the uncertainty for the $SSB$ value for a point means we do not know the true value at that point, and so we may want to sample the same point again to become more confident in what the true $SSB$ value is at that point. The EI acquisition function does not allow this, as the EI of a point becomes zero once you have sampled it. Hence, if we make sure we are using an acquisition function that can model sampling noise such as AEI or KG, we can continue the framework as beforehand, whilst accounting for the uncertainty of the $SSB$ values.
