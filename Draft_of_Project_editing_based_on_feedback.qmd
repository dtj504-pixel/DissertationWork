...
RE-READ BEFORE SUBMISSION BUT MOSTLY FOCUS ON ANALYSING RESULTS SECTION FROM NOW ON
...
In the @sec-singlestock part of the project, we build on @Originalpaper. This paper is looking to find the HCR parametrised by $F_{target}$ and $B_{trigger}$ that maximises the median long-term catch whilst keeping the risk precautionary for a single-stock fishery [@Originalpaper]. The paper uses many different methods to achieve this, which we investigate in detail below and apply to our own work. We have focused on enhancing these methods by adding acquisition functions and k-means clustering to improve how we choose to sample points between rounds as this was not deeply considered in the original paper [@Originalpaper],[@BayesianOptimisationTutorial],[@EfficientGlobalOptimizationofExpensiveBlackBoxFunctions],[@GlobalOptimizationofStochasticBlackBoxSystemsviaSequentialKrigingMetaModels]. We demonstrate clearly how these methods become applicable in our own work in @sec-application.

The aim of the second half of the project in @sec-mixedfisheries is to apply the methods from @sec-singlestock to mixed fisheries. We use the methods from the first half of the project but change the aim to finding the $F_{target}$ for each stock that is precautionary but maximises long-term catch. We focus on two datasets and receive results for each by running the simulations on a High Performance Computer because the objective function is no longer pre-evaluated as it was in @Originalpaper. This means we have applied our method to the more realistic situation of a mixed fishery and have been able to demonstrate how effective our method may be if used in the industry [@BayesianOptimisationTutorial],[@MixME],[@MixedFisheriesStatusandManagement2023]. These results are discussed in @sec-analysingresults.
...
-   We use the GP modelling $\ln(risk)$ to get the value for the risk at each point in the sample space
...
For a comprehensive introduction to the general theory on Gaussian processes, Maximum Likelihood Estimation and Gaussian Process Regression see @BayesianOptimisationTutorial.
...
This is equivalent to saying that if for some $x,x',x''$ in the design space we have $\|x - x'\| < \|x -x''\|$ for some norm $\| \cdot \|$, then $\operatorname{cov}_0(x, x') > \operatorname{cov}_0(x, x'')$.

We use a GP to emulate the objective function because it is much cheaper to evaluate than our objective function. We can use the GP $F(x)$ for any $x$ in the sample space as our estimate of $f(x)$ based on our current beliefs. - UNCLEAR
...
We can calculate this distribution for every point in the design space $X$. This results in a new GP $F_n$ with a mean vector and covariance kernel that depend on the location of the unevaluated points, the locations of the evaluated points $x_{1:n}$, and their values $f(x_{1:n})$. So, we can update our GP every round based on the new points we have evaluated.

### Bayesian History Matching

For the theory here, please see @Originalpaper.

Let $x$ be a point in the sample space. We begin with some uncertainty about our objective function $f(x)$. However, we can make probabilistic statements such as:
...
We now let $a=max\{f(x_1),...,f(x_n)\}$ where $n$ is the number of points we have evaluated so far. As the rounds increase we make sure to include all previous points of the objective function that have been evaluated in $n$. We remove the point $x$ if:
...
We remove the assumption of EI that we have to return a pre-evaluated point as our best point [@BayesianOptimisationTutorial]. We also now start by saying that the solution we would choose if we have to stop sampling after $n$ points would be the point in the design space with the largest $\mu_n(\cdot)$ value, where $\mu_n(\cdot)$ is the mean vector of the posterior probability distribution after $n$ iterations. - WHY IS mu_n A VECTOR?
...
To do this, @Originalpaper uses two GPs where the risk GP models $\ln(risk)$ and the catch GP models $\ln(median\ catch)$. Maintaining the notation from @Originalpaper, we use $m_1$ for the mean function of the catch GP and $m_2$ for the mean function of the risk GP:
...
For an - WHAT? IN DEPTH DISCUSSION? OF WHAT? MAYBE PROPERTIES AND USES OF DIFFERENT KERNELS AND MEANS BUT NEED TO CHECK DOCUMENT FIRST - see @williams2006gaussian.
...
CHECK THE RISK EQUATION BELOW - DEFINETLY NOT SUPPOSED TO BE \< BUT ENSURE I HAVE CHANGED IT CORRECTLY
...
We chose our first point to sample as $F_{cod} = 0.28$ and $F_{had} = 0.353$ based upon the values given in the Fixed fishing mortality management strategy example from the MixME wiki [@MixMEwiki]. These were drawn from North Sea Cod and Celtic Sea Haddock advice [@ICES2021Cod028],[@ICES2020HaddockBlim]. We still sampled $n-2$ other points in the first round as well.
...
FULLY READ UP ON CITTAIONS IN PARGRAGH BELOW
