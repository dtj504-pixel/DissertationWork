...
$$
\small g(\theta) = I_{[Cod\ B_{lim},\infty)}(P(\textrm{min}(SSB_{Cod})<B_{lim})) \times I_{[Haddock\ B_{lim},\infty)}(P(\textrm{min}(SSB_{Haddock})<B_{lim})) \times C(\theta) 
$$where the indicator functions are:

$$
I_{[0.95,1]}(P(\textrm{min}(SSB_{Cod})<B_{lim})) = \begin{cases} 1 & \text{if } \textrm{min}(SSB_{Cod})\in[Cod\ B_{lim},\infty) \\ 0 & \text{otherwise } \end{cases} 
$$
