...
Another area for improvement would be considering other metrics to optimise. For example, instead of maximising for long-term median catch, I could aim for Maximum Economic Yield which also takes into account the effort expended by the fleet to catch the stock. We could also consider the change in TAC. It is common for fisheries to want a relatively stable TAC so that they can predict their finances for the years ahead. This is occasionally taken into consideration by ICES.

Trophic interactions (interactions between species in a mixed fishery) are not considered in the MixME model. However, there is evidence to suggest that these are still important factors even in areas dominated by fishing activity (?). This means we should be inputting this data into our model as well to provide better projections into the future. EVENIF NOT IN AREAS DOMINATED BY FISHING, COULD REASON THAT SOME MIXED FISHERIES AREN'T AND SO COULD BE USEFUL IN THEIR CASES
