...
In the literature, this method can be understood through the lens of Constrained Bayesian Optimisation. We are using BHM to determine which points are possible based on:

$$
P(C(\theta)>C(\theta^*)) > 0.0001 \ \textrm{and} \ P(risk\le0.05) > 0.0001
$$

where $\theta,\theta^*$ are points in the parameter space and $\theta^*$ gives our current maximum long-term median catch as $C(\theta^*)$. The points meeting these conditions will be called the Possible Space, $PS$. Then, within $PS$ our acquisition function searches for the optimal parameters. It is important to reassess these constraints each round because as we gain more information different sets of points may be included in $PS$.
