...
Throughout this section, we often call $f$ an objective function as this is the standard across the literature. It should be regarded as separate from our objective function in @eq-objectivefunctionsinglestock, which clearly needs to be split into separate parts to be emulated by a Gaussian Process (GP). Until the end of the @sec-generaltheory section, $f$ should be regarded as an objective function we wish to maximise which is emulated by a GP.
...
We choose a covariance function such that if for some $x,x',x''$ in the parameter space we have $\|x - x'\| < \|x -x''\|$ for some norm $\| \cdot \|$, then $\operatorname{cov}_0(x, x') > \operatorname{cov}_0(x, x'')$. We use a GP to emulate the objective function because it is much cheaper to evaluate than our objective function. We can use the GP $F(x)$ as our belief of the distribution of values for $f(x)$ for any point $x$ in the parameter space. This is true even for the evaluated points $x_1,...,x_m$ as the GP is fitted to these points [@Originalpaper].

### Maximum Likelihood Estimation

When using GPs to emulate our objective function, we need to be able to estimate the hyperparameters of the GP using the data we gain from evaluating our points $x_1,...,x_n$. We can do this as follows. Firstly, we let the vector $\eta$ represent the hyperparameters of the GP. Then, given the observations $f(x_{1:n}) = (f(x_1),...,f(x_n))$, we calculate the likelihood of these observations under the prior distribution given $\eta$ which is denoted as $p(f(x_{1:n})|\eta)$ and modelled by @eq-MultivariateNormalDist. - UNDER THE PRIOR IS UNCLEAR
...
This conditional distribution $F(x_{n+1})|f(x_{1:n})$ is called the posterior probability distribution. We can calculate this distribution for every point in the parameter space $X$ by changing the value of $x_{n+1}$. This results in a new GP $F_n$ with a mean vector and covariance kernel that depend only on the location of the unevaluated points, the locations of the evaluated points $x_{1:n}$, and their values $f(x_{1:n})$. So, we can update our GP every round based on the new points we have evaluated.

However, in our framework we have chosen instead to create a new GP each round based on the current data we have instead of updating our GP from the previous round as above. Despite this, we do use @eq-posteriordistributiongivensamples to find $F(x_{n+1})|f(x_{1:n})$\$ for each point $x_{n+1}$ in the parameter space that we have not already sampled. This then allows us to move on to using BHM as we have probabilities for the value of $f(x_{n+1})$ from the distribution.
...
Firstly, we extract the $\textrm{min}(SSB)$ for each stock from the result each of the simulations we have run [@MixMEwiki],[@MixME]. We then put these results into the GPs $\textrm{min}(SSB_{Cod})$ and $\textrm{min}(SSB_{Haddock})$ Next, we predict the $\textrm{min}(SSB)$ for every point in the parameter space for and calculate:
