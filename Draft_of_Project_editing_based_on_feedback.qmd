...
This conditional distribution $F(x_{n+1})|f(x_{1:n})$ is called the posterior probability distribution. We can calculate this distribution for every point in the parameter space $X$ by changing the value of $x_{n+1}$. This results in a new GP $F_n$ with a mean vector and covariance kernel that depend on the location of the unevaluated points, the locations of the evaluated points $x_{1:n}$, and their values $f(x_{1:n})$. So, we can update our GP every round based on the new points we have evaluated.
