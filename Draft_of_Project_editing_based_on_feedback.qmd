...
From running our simulation based on the fixed fishing mortality example o n the MixME wiki we receive similar results, despite using the same dataset (?).  - IS IT?

Again, we obtain a list of parameters and are able to determine because cod is the choke stock that $F_{cod} = 0.38$ and $F_{had} = 0.28$ are the optimal parameters [@MixMEwiki]. The $F_{cod}$ falls slightly closer to the $F_{MSY}$ value recommended by ICES and is still below $F_{PA}$ [@ICES2025CodAdvice]. The $F_{had}$ is again below both the $F_{MSY}$ and the $F_{PA}$, suggesting haddock is being choked by cod [@ICES2025HaddockAdvice]. The values from this model are likely to be different to those from the shortcut model because they have been set up in different way.  - FINDING SOME EXAMPLES WOULD BE GOOD (For example, the $SSB$ is taken without any observation error in the shortcut model - does this mean the pre-evaluated noise is not included? SENT EMAIL TO MATT).
...
HOW DO THESE RELATE TO MY RESULTS FROM THE GRID SEARCH?

I then conducted a grid search of the whole parameter space. From this, I was able to find that the optimal parameters are
...
Furthermore, as there are 1000 iterations in this dataset, I will then be able to do the risk calculation as described in @MixME and as is it is typically done across the industry [@MixMESupp],[@ICES2019WKGMSE2].

THIS  - show that I know what this is - equations
