...
We first create the operating model `mixedfishery_MixME_om` which contains the true data for the stocks and fleets in the dataset [@MixME]. In both of our files, this is loaded in with the dataset [@MixMEwiki]. This true data is generated using standard age-structured equations to model the dynamics of the fleets and the stocks [@MixMEwiki]. For every year, it gives as a result the catch (in terms of landings and discards) from each fleet and the survivors from that year. Recruitment is done at the beginning of each time step [@MixMEwiki].
