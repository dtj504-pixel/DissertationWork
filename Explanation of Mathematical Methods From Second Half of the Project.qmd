...
We started with $F_{cod} = 0.28$ and $F_{had} = 0.353$ based upon the values given in the Fixed fishing mortality management strategy example from the MixME wiki [@MixMEwiki]. These were drawn from the North Seas Cod and Celtic Seas Haddock advice published in 2020 respectively - ASKED MIKE ABOUT CITATION FOR COD AS CAN'T SEE 0.28 ANYWHERE - ,[@ICES202HaddockBlim].

### Creating the Operating model

We first create the operating model `mixedfishery_MixME_om` which contains the true data for the stocks and fleets in the dataset [@MixME]. For the stocks, this includes the numbers, natural mortality, stock mean individual weight and proportion mature and for fleets this includes landing numbers, landing mean individual weight, discard numbers, discard mean individual weight, selectivity and catchability**.**

In both of our files, this is loaded with the dataset [@MixMEwiki]. This true data is generated using standard age-structured equations to model the dynamics of the fleets and the stocks [@MixMEwiki]. For every year, it gives as a result the catch (in terms of landings and discards) from each fleet and the survivors from that year - FIND THIS AGAIN. Recruitment is done at the beginning of each time step [@MixMEwiki].

MOST OF THE BELOW SHOULD BE IN THE WIKI EXAMPLE

We use this in the makeMixME function to make the MixME model. We also use it to determine the quota share of the catch for each fleet in the calculateQuotashare function. WE also use it ot project the fishery forwards in the stfMixME function.

We also use it to set and arbitrary effort target for each fleet in 2020, as we only care about the values at the beginning of 2020. We also use it to make the FCB matrix which shows which fleets catch which stocks.

We use it in the FLasher::fwd projection were we simulate one year of fishing to get the starting conditions right.

After this, we update it with the projected population numbers for the end of 2020.

WE use it with FLSTocks to make the make the observation error model, stk_oem.
