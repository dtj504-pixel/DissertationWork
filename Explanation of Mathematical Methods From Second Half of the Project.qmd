...
This process has been set up so that it is easy to run in parallel. We are able to load in the data we need once and then run the simulations for each point we have chosen to sample in the round. Then, we collect the results from these simulations which consist of the $min(SSB)$ for cod, the $min(SSB)$ for haddock and the total catch of cod and haddock all over the years 2030-2039.

These results allow us to set up the GP for

and are able to use them  - MAIN PART I PARALLELLISE before running multiple jobs at tehs ame time
