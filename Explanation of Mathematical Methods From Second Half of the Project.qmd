...
stk_oem <- FLStocks(lapply(mixedfishery_MixME_om$stks, function(x) {
  ...
  catch <- sapply(mixedfishery_MixME_om$flts, function(y) which(names(y) 
           %in% name(x)))
...
Lastly, we update some of the management and simulation settings and run the simulation [@MixMEwiki].
...
It can be seen above that we have updated the arguments for when catches occur, the age range for calculating average fishing mortality in all the places it is needed and the target fishing mortality we are using for this round of the simulation. - MAYBE SIMILAR ON WIKI
