...
Setting the mean of the GP to zero as in @lst-mean_zero is more often done in examples to showcase the key concepts than when implementing these methods in real-world situations. - GET CITATION My experiments seem to back up this theory as they again show results that are the same or worse than our original approach.
...
We can see from the heatmaps below that the simulations with means changed to zero either converge at the same rate or worse than when we use the original mean specified in [@OriginalPaper].
