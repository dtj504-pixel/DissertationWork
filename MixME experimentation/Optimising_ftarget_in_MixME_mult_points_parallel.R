...
# CHANGED: Pick 7 other RANDOM points from your grid to initialize the model with a set seed for reproducibility
# TODO: Could space apart equally as in Mike's code to explore the space more 
set.seed(123) 
warmup_indices <- sample(nrow(dat), 7)
...
for (iteration in 1:max_rounds) {
  ...
  # FIRST CHANGE: multiple candidate points
  # Get 8 candidate points with highest KG
  # Select next points
  if (nrow(cand) <= 8) {
    ...
    set.seed(123)
    km_result <- kmeans(top_candidates[, c("Fcod", "Fhad")], centers = 8)
  ...
  # ONly need next_points now that we picking from rows in a grid
