...
for (iteration in 1:max_rounds) {
    ...
    # Calculate Total Catch for Cod (2020-2039)
    # We filter to find only fleets that have a "cod" component
    flts_with_cod <- Filter(function(f) "cod" %in% names(f), res$om$flts)

    # Sum catches from those fleets
    catch_cod_quant <- Reduce("+", lapply(flts_with_cod, function(f) catch(f[["cod"]])))

    # Sum over the specific years (2020 to 2039)
    total_catch_cod <- sum(catch_cod_quant[, ac(2020:2039)])


    # Calculate Total Catch for Haddock (2020-2039)
    flts_with_had <- Filter(function(f) "had" %in% names(f), res$om$flts)
    catch_had_quant <- Reduce("+", lapply(flts_with_had, function(f) catch(f[["had"]])))
    total_catch_had <- sum(catch_had_quant[, ac(2020:2039)])

    # Combine to get Total Catch for both species
    ...
    # Define Blims here too for convenience
    Blim_cod <- 107000
    Blim_had <- 9227

    # Extract SSB directly from the tracking slot using "SB.om"
    # We select the metric "SB.om" and the years 2020 to 2039
    ...
    # This checks if SSB < Blim (TRUE/FALSE) and averages across iterations
    # Currently, we only have 1 iteration, the result will be 0 (Safe) or 1 (Collapsed)
    risk_cod_annual <- iterMeans(ssb_cod_data < Blim_cod)
    risk_had_annual <- iterMeans(ssb_had_data < Blim_had)

    # Extract the specific risk for the final year (2039)
    # We use c() to drop dimensions and get a clean number
    ...
    # SET UP THE GPS
    gp_log_cat <- km(~.^2,design=runs[,c("Fcod","Fhad")],estim.method="MLE",response = runs$log_total_catch,nugget=1e-12*var(log_total_catch),covtype = "exp")
    gp_cod_risk <- km(~.^2,design=runs[,c("Fcod","Fhad")],estim.method="MLE",response = runs$risk_cod,nugget=1e-12*var(risk_cod),covtype = "exp")
    gp_had_risk <- km(~.^2,design=runs[,c("Fcod","Fhad")],estim.method="MLE",response = runs$risk_had,nugget=1e-12*var(risk_had),covtype = "exp")
    ...
    valid_runs <- runs$Total_Catch[runs$RiskCod < 0.05 & runs$RiskHad < 0.05]
