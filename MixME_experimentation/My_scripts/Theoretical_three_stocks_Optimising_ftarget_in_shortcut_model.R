...
obj_func <- function(f_cod, f_had, f_whg, input) {
    ...
    input$ctrl_obj$hcr@args$hcrmethod$whg <- ICES_HCR 
    ...
    input$ctrl_obj$isys@args$isysmethod$whg <- forecast_fun
    
    # SET PARAMETERS for the HCR
    # TODO: I need to guess the Blim/Btrigger for Whiting or use placeholders. Please replace these values with the real ones if you have them!
    input$ctrl_obj$phcr <- mseCtrl(args = list(hcrpars = list(
        ...
        whg = c("Ftrgt" = f_whg, "Btrigger" = 20000, "Blim" = 15000) # EXAMPLE VALUES
    )))
    ...
    input$ctrl_obj$isys@args$isysList <- list(
        ...
        whg = list(fwd_trgt = c("fsq", "hcr"), fwd_yrs = 2, fwd_yrs_fsq = -2:0, fwd_yrs_average = -2:0, fwd_yrs_rec_start = 2010, fwd_yrs_sel = -3:-1)
    )
    ...
    input$args$adviceInit$whg[] <- 1000
...
# Define funciton that will get risk and catch for each version of f_cod, f_had, f_whg
doOne <- function(run_id,input_data){
    ...
    this_Fwhg <- input_data$Fwhg[run_id]
    
    # Run the simulation
    res <- obj_func(f_cod = this_Fcod, f_had = this_Fhad, f_whg = this_Fwhg, input = input)
    ...
    catch_whg <- sum(res$tracking$whg$stk["C.om", ac(2020:2039)], na.rm = TRUE)
    # Calculate total catch
    total_catch <- catch_cod + catch_had + catch_whg
    ...
    # TODO: Arbitrary Blim for whiting, should change
    Blim_cod <- 107000
    Blim_had <- 9227
    Blim_whg <- 15000
    ...
    ssb_whg_data <- c(res$tracking$whg$stk["SB.om", ac(2039)])
    ...
    if (is.na(ssb_whg_data)) {
        # Maximum penalty as -1 means we have crashed to SSB = 0
        risk_whg <- -1 
    } else {
        risk_whg <- (ssb_whg_data - Blim_whg) / Blim_whg
    }
    
    # Set up variable to return
    to_return <- c(Fcod = this_Fcod,Fhad = this_Fhad,Fwhg = this_Fwhg,RiskCod = risk_cod,RiskHad = risk_had,RiskWhg = risk_whg,TotalCatch = total_catch)
    
    # Return the result
    return(to_return)
}

# Calculate how correlated two vectors of Fcod, Fhad and Fwhg are based on their distance in the grid
...
knowledge_gradient_sim <- function(mu, sigma, model, obs_noise_var = 0, nsim = 100, prisk_cod, prisk_had, prisk_whg, eps = 1e-4) 
{ 
  X_pred <- dat[, c("Fcod","Fhad", "Fwhg")]
  ...
  for (i in seq_len(m)) {
    ...
    if (prisk_cod[i] > eps || prisk_had[i] > eps || prisk_whg[i] > eps) {
      kg[i] <- 0
      next
    }
...
# TDOD: Change initial Fwhg to something sensible when known
f_cod_1 <- 0.28
f_had_1 <- 0.353
...
for (iteration in 1:max_rounds) {
    ...
    names(data_frame_result) <- c("Fcod","Fhad","Fwhg","RiskCod","RiskHad","RiskWhg","TotalCatch","error","warning","time")
    ...
    names(dat_run) <- c("Fcod","Fhad","Fwhg","RiskCod","RiskHad","RiskWhg","TotalCatch")
    ...
    if(var(risk_whg_input, na.rm = TRUE) == 0) {
        risk_whg_input <- risk_whg_input + rnorm(length(risk_whg_input), mean=0, sd=1e-9)
    }
    
    # SET UP THE GPS
    # Adding 1e-15 to nuggets to avoid 0 nugget variance
    # TODO: remove when risk calculations fixed
    gp_log_cat <- km(~.^2,design=runs[,c("Fcod","Fhad","Fwhg")],estim.method="MLE",response = log_total_catch,nugget=1e-12*var(runs$TotalCatch)+1e-15,covtype = "exp")
    gp_cod_risk <- km(~.^2,design=runs[,c("Fcod","Fhad","Fwhg")],estim.method="MLE",response = risk_cod_input,nugget=1e-12*var(risk_cod_input)+1e-15,covtype = "exp")
    gp_had_risk <- km(~.^2,design=runs[,c("Fcod","Fhad","Fwhg")],estim.method="MLE",response = risk_had_input,nugget=1e-12*var(risk_had_input)+1e-15,covtype = "exp")
    gp_whg_risk <- km(~.^2,design=runs[,c("Fcod","Fhad","Fwhg")],estim.method="MLE",response = risk_whg_input,nugget=1e-12*var(risk_whg_input)+1e-15,covtype = "exp")
    ...
    pred_risk_whg <- predict(gp_whg_risk, newdata = dat, type = "SK")
    ...
    risk_threshold_whg <- -1000 / Blim_whg    # approx -0.067   
    ...
    prisk_whg <- pnorm(risk_threshold_whg, pred_risk_whg$mean, pred_risk_whg$sd + 1e-12)
    
    # Filter for valid runs
    valid_runs <- runs$TotalCatch[runs$RiskCod > risk_threshold_cod & runs$RiskHad > risk_threshold_had & runs$RiskWhg > risk_threshold_whg]
    ...
    possible <- pmin( (1 - pcat), (1 - prisk_cod), (1 - prisk_had), (1-prisk_whg), 1 ) > eps
    ...
    kg <- knowledge_gradient_sim(mu, sigma, gp_log_cat, obs_noise_var = 0, nsim = 100, prisk_cod = prisk_cod, prisk_had = prisk_had, prisk_whg = prisk_whg, eps = 1e-4)
    ...
    names(dat_with_kg) <- c("Fcod","Fhad","Fwhg","kg","possible")
    
    # Filter to only plausible points with positive KG
    cand <- subset(dat_with_kg, possible == TRUE & kg > 0)
    
    # Create keys for filtering
    cand$key <- paste(cand$Fcod, cand$Fhad, cand$Fwhg, sep = "_")
    runs$key <- paste(runs$Fcod, runs$Fhad, cand$Fwhg, sep = "_")
    ...
    if (nrow(cand) <= 6) {
        next_points <- cand[order(-cand$kg), c("Fcod", "Fhad", "Fwhg")]
    } else {
        top_candidates <- cand[order(-cand$kg), ][1:nrow(cand), ]
        set.seed(123)
        km_result <- kmeans(top_candidates[, c("Fcod", "Fhad", "Fwhg")], centers = 6)
        ...
        df[which.max(df$kg), c("Fcod", "Fhad", "Fwhg", "kg")]
        }))
    }
    ...
    rm(gp_log_cat, gp_cod_risk, gp_had_risk, gp_whg_risk)
    
    # Remove the prediction vectors to be safe
    rm(pred_risk_cod, pred_risk_had, pred_risk_whg, pred_log_cat)
    ...
    important_cols <- c("Fcod","Fhad","Fwhg","RiskCod","RiskHad","RiskWhg","TotalCatch")
